...
-- 00065 EFFECTIVE STATUS, PROOF FOLDER
-- 1. fn_invoice_effective_status is the one Terlambat rule: a stored overdue,
--    or a draft or sent whose due date has passed. The invoice list, the
--    invoice summary and the dashboard all read it, so a legacy stored
--    overdue with a future due date is counted the same way everywhere.
--    It is a single-expression SQL function, so the planner inlines it.
-- 2. fn_change_invoice_status only records a payment proof stored under
--    invoices/<id>/payment/, the folder the proof upload URL writes to, so
--    an invoice attachment or another invoice's upload cannot pass as one.

-- +goose StatementBegin
CREATE FUNCTION fn_invoice_effective_status(p_status text, p_due_date date)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
...
DROP FUNCTION IF EXISTS fn_invoice_effective_status(text, date);
