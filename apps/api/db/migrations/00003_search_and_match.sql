...
-- 4. Function: fn_match_request — smart match with learning cache
-- 5. Trigger:  trg_learn_match — UPSERT item_request_matches
-- on each quotation_item INSERT with match confirmed
...
-- idx_items_name_trgm supports both `<%` operator and `ILIKE` search.
...
COMMENT ON FUNCTION fn_search_items(TEXT, REAL, INT) IS
  'Combined-signal fuzzy search. Tiers: >=0.90 AUTO_MATCH, >=0.60 SUGGESTED, >=0.30 FUZZY. Normalizes via UPPER(TRIM+collapse whitespace).';
-- +goose StatementEnd
...
COMMENT ON FUNCTION fn_search_items_by_vendor(BIGINT, INT) IS
  'Lists all active items supplied by a vendor, sorted by last_quoted_at DESC.';
-- +goose StatementEnd
