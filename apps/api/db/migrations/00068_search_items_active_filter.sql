...
-- 00068 SEARCH ITEMS ACTIVE FILTER
-- fn_search_items kept only active items, so the Katalog search could not
-- find a deactivated product by name, not even under the Nonaktif filter.
-- p_is_active now picks the rows: TRUE (the default) keeps every existing
-- caller active-only, FALSE keeps inactive items, NULL keeps both.
-- The old three-argument signature is dropped first: a second overload
-- would make every three-argument call ambiguous.

DROP FUNCTION public.fn_search_items(text, real, integer);

-- +goose StatementBegin
CREATE FUNCTION public.fn_search_items(p_q text, p_min_score real DEFAULT 0.3, p_limit integer DEFAULT 10, p_is_active boolean DEFAULT true)
...
DROP FUNCTION public.fn_search_items(text, real, integer, boolean);

-- +goose StatementBegin
CREATE FUNCTION public.fn_search_items(p_q text, p_min_score real DEFAULT 0.3, p_limit integer DEFAULT 10)
 RETURNS TABLE(id bigint, name character varying, impa_code character varying, default_unit_id smallint, score real, match_tier text)
 LANGUAGE sql
 STABLE
AS $function$
