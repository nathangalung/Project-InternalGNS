...
-- For Detail page (status timeline) and Edit page (atomic update).
-- Table:
-- * quotation_status_history (id, quotation_id, from→to, note, changed_by, changed_at)
-- Trigger:
-- * trg_log_quotation_creation — AFTER INSERT, log first entry
-- (from=NULL, to=initial_status). FE timeline gets "Created" entry.
-- Functions:
-- * fn_change_quotation_status(id, new_status, user_id, note)
-- UPDATE quotations.status + INSERT history in 1 transaction.
-- * fn_update_quotation(id, payload..., user_id)
-- Atomic edit (DRAFT only): UPDATE header + DELETE items + INSERT items.
-- Mirrors fn_create_quotation but UPDATE pattern. Triggers inherit
-- discount_pct + sync vendor cost still fire from INSERT items.
-- Backfill: existing quotations get 1 initial entry (from=NULL) so
-- FE timeline is not empty for historical data.


-- 1. Table quotation_status_history.
...
BEGIN
  ...
  -- UPDATE header (triggers set_updated_at + bump row_version fire)
  UPDATE quotations
...
COMMENT ON FUNCTION fn_change_quotation_status(BIGINT, TEXT, BIGINT, TEXT) IS
  'Atomic status change: UPDATE quotations.status + INSERT quotation_status_history in 1 transaction. Idempotent (no-op if same status). Used by PATCH /quotations/{id}/status.';
-- +goose StatementEnd
...
COMMENT ON FUNCTION fn_update_quotation(
  ...
  'Atomic edit quotation (DRAFT only): UPDATE header + DELETE items + INSERT new items + optional shipping. Blocks edit when status != draft. Used by PUT /quotations/{id}.';
-- +goose StatementEnd


-- 5. BACKFILL: add initial entry for existing quotations
-- +goose StatementBegin
