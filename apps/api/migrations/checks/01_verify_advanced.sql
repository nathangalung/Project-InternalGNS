...
-- Jalanin per section (paste ke psql / pgAdmin)
-- ============================================================
-- 8 kategori:
--   A. Schema Introspection (struktur DB)
--   B. Business Logic Cross-Check (GENERATED columns akurat?)
--   C. Negative Tests (pastikan constraints enforce — harus ERROR)
...
-- A.1 Semua tabel + row count + size
SELECT
...
-- A.3 Semua CHECK constraints (enum + range)
SELECT
...
-- A.4 Semua GENERATED columns (harus ada ~15 rows)
SELECT
  table_name,
  column_name,
  data_type,
...
-- A.5 Semua triggers (row_version + updated_at)
SELECT
...
-- A.6 Semua indexes grouped per tabel
SELECT
  tablename,
  indexname,
  indexdef
...
--    (Compute manually di SQL, compare dengan GENERATED)
-- ═══════════════════════════════════════════════════════════

-- B.1 Verify quotation_items GENERATED columns (per baris)
SELECT
  line_number,
  qty, selling_price, cost_price, item_type,
  total_selling,
  (qty * selling_price) AS expected_total_selling,
  total_selling - (qty * selling_price) AS diff_total_selling,   -- harus 0
...
SELECT
  ...
  grand_total - (dpp_nilai_lain + ppn_amount) AS diff_grand_vs_dpp_plus_ppn  -- harus 0 (tolerance rounding)
FROM quotations;
...
-- ═══════════════════════════════════════════════════════════
-- C. NEGATIVE TESTS (paste satu-per-satu — harus ERROR)
--    Pakai BEGIN...ROLLBACK supaya tidak corrupt data
-- ═══════════════════════════════════════════════════════════
...
-- C.2 Invalid ppn_rate (harus = 12.00)
BEGIN;
...
-- C.6 RESTRICT on DELETE company_client (dipakai by quotation)
BEGIN;
...
-- C.7 CASCADE on DELETE quotation (items harus ikut terhapus)
BEGIN;
DELETE FROM quotations WHERE id = 1;
-- Ini akan ERROR kalau ada PO yang reference (RESTRICT) — expected behavior
-- Kalau mau test cascade: hapus PO/invoice dulu di draft DB
ROLLBACK;

-- C.8 ON DELETE SET NULL (delete contact → quotation.contact_id jadi NULL)
BEGIN;
...
-- ═══════════════════════════════════════════════════════════
-- D. SNAPSHOT DRIFT DETECTION
--    Cek apakah snapshot fields masih match master (should match
--    untuk data baru, bisa drift untuk data historis — expected)
-- ═══════════════════════════════════════════════════════════
...
-- F.1 Orphan snapshot — quotations yang FK company masih valid?
...
-- F.2 Quotation tanpa items (draft kosong?)
SELECT q.id, q.quotation_no, q.status
FROM quotations q
LEFT JOIN quotation_items qi ON qi.quotation_id = q.id
WHERE qi.id IS NULL;

-- F.3 Quotation accepted tanpa PO (pending conversion)
...
-- F.4 PO DELIVERED tanpa invoice (pending billing)
...
-- F.5 Items yang tidak punya vendor_product (tidak bisa di-source)
...
-- F.6 vendor_products aktif tapi vendor/item sudah inactive
...
-- F.7 Snapshot NULL yang seharusnya NOT NULL
...
-- G.1 Index usage stats (identifikasi index yang tidak terpakai)
SELECT
...
-- idx_scan = 0 → index tidak pernah dipakai (kalau sudah ada traffic, candidate untuk drop)
...
-- G.3 Fuzzy search via pg_trgm (test GIN index)
...
ORDER BY score DESC
LIMIT 5;
-- Cek Planning Time + Execution Time. Harus pakai "Bitmap Index Scan on idx_items_name_trgm"

-- G.4 Slow query untuk status filter (cek idx_quotations_status_created)
EXPLAIN (ANALYZE)
...
LIMIT 20;
-- Harus pakai "Index Scan using idx_quotations_status_created"
...
-- H.6 Vendor performance — revenue yang datang dari vendor X
SELECT
  v.name AS vendor,
...
-- H.7 Running number usage — cek gap di sequence
WITH parsed AS (
  SELECT
    id, quotation_no,
...
GROUP BY seq_str
HAVING COUNT(*) > 1;  -- Deteksi duplicate sequence (shouldn't happen with UNIQUE)

-- H.8 Learning table — request text yang paling sering match
SELECT
  request_text,
...
UPDATE quotations SET notes = 'stale_write' WHERE id = 1 AND row_version = 0;  -- ganti 0 dgn nilai lebih kecil dari real
...
-- Misal row_version = 1, pakai nilai itu:
UPDATE quotations SET notes = 'fresh_write'
WHERE id = 1 AND row_version = 1;  -- GANTI dgn actual row_version
-- Expected: 1 row affected, row_version auto-increment ke 2 via trigger
SELECT row_version FROM quotations WHERE id = 1;
ROLLBACK;


-- ═══════════════════════════════════════════════════════════
-- CLEANUP: Reset sample data kalau perlu
-- ═══════════════════════════════════════════════════════════
-- TRUNCATE invoice_items, invoices, purchase_order_items, purchase_orders,
--          quotation_items, quotations, item_request_matches
--   RESTART IDENTITY CASCADE;
-- (master data tetap — hanya transactions yang di-reset)
