...
-- ============================================================
-- Tabel sequence per (doc_type, company_id, year). UPSERT atomic
-- via ON CONFLICT memastikan no duplicate sequence saat concurrent.
--
-- Format quotation/PO/invoice number:
--   {prefix}-{YY}{company_number}{seq}/GNS/{Roman month}/{YYYY}
--
-- Contoh: Q-26264101/GNS/IV/2026
--   prefix         = Q
--   YY             = 26 (year 2-digit)
...
COMMENT ON TABLE doc_sequences IS
  'Running sequence per (doc_type, company, year). UPSERT atomic via fn_next_doc_no untuk race-safe number generation.';
...
BEGIN
  -- Get company number untuk format
  ...
  -- UPSERT atomic: increment seq atau insert baru
...
COMMENT ON FUNCTION fn_next_doc_no(VARCHAR, BIGINT) IS
  'Atomic next doc number untuk Q/PO/INV/DN. Format: {prefix}-{YY}{company_no}{seq}/GNS/{Roman}/{YYYY}. Race-safe via UPSERT pada doc_sequences.';
-- +goose StatementEnd
