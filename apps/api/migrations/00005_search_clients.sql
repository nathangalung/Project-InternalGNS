...
-- ============================================================
-- Fuzzy search di company_client + company_contacts.
-- Output: flat row per (company × contact). Company tanpa
-- contact aktif tetap muncul dengan contact_* = NULL (LEFT JOIN).
-- Search signals: company name, contact name, email.
-- (NPWP excluded per user decision).
-- ============================================================

-- Indexes untuk fuzzy search (data UPPERCASE convention)
...
COMMENT ON FUNCTION fn_search_clients(TEXT, REAL, INT) IS
  'Fuzzy search company + contacts. Flat 1 row per (company × contact). Company tanpa contact aktif tetap muncul (contact_* NULL). Signals: company.name (0.95), contact.name (0.85), email (0.80). Tier: AUTO_MATCH ≥0.9, SUGGESTED ≥0.6, FUZZY ≥0.3.';
-- +goose StatementEnd
