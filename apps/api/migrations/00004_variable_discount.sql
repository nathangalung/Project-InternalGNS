...
-- ============================================================
-- Replace hardcoded 5% (0.05) di quotation_items + po_items
-- GENERATED columns dengan variable discount_pct.
--
-- Rules:
...
-- * Header.discount_pct UPDATE blocked kalau status != 'draft'
--   → kalau mau ubah, harus bikin quotation versi baru (revision system)
-- * Cascade ke items hanya fire pada draft (block dulu di BEFORE trigger)
...
-- ─── 2. Backfill existing rows dengan 0.05 (preserve angka existing) ──
...
-- ─── 4. Drop & recreate GENERATED columns (use new discount_pct) ──
-- NOTE: quotation_reconciliation view depends on quotation_items.discount_amount,
-- so harus drop view dulu, recreate setelah column rebuilt.
...
-- Recreate view yang sempat di-drop di step 4
...
-- ─── 5. BEFORE INSERT trigger: quotation_items inherit dari header ──
-- +goose StatementBegin
...
-- ─── 6. BEFORE INSERT trigger: po_items inherit dari PO header ──
-- +goose StatementBegin
...
-- ─── 7. BEFORE INSERT trigger: PO inherits dari quotation ──
-- +goose StatementBegin
...
-- ─── 8. PROTECT: block discount_pct UPDATE kalau status != 'draft' ──
-- +goose StatementBegin
...
-- ─── 9. CASCADE: quotation_pct change → propagate ke items ──
-- +goose StatementBegin
...
COMMENT ON COLUMN quotations.discount_pct IS
  'Diskon % untuk seluruh item quotation ini (NUMERIC 0..1, e.g. 0.05 = 5%). Tidak ada DB DEFAULT — app layer wajib set saat INSERT. Setelah status != draft, UPDATE diblokir trigger trg_protect_quotation_discount.';

COMMENT ON COLUMN quotation_items.discount_pct IS
  'Snapshot discount_pct dari quotations header. Auto-inherit via trg_inherit_quotation_discount BEFORE INSERT. Header UPDATE cascades via trg_cascade_quotation_discount.';

COMMENT ON COLUMN purchase_orders.discount_pct IS
  'Diskon % untuk PO ini, snapshot dari quotation pada saat PO dibuat. Auto-inherit via trg_po_inherit_quotation_discount.';

COMMENT ON COLUMN purchase_order_items.discount_pct IS
  'Snapshot dari purchase_orders.discount_pct, auto-inherit via trg_inherit_po_discount.';
-- +goose StatementEnd
...
-- Drop view (depends on discount_amount yang akan di-revert)
DROP VIEW IF EXISTS quotation_reconciliation;

-- Revert GENERATED ke hardcoded 0.05
ALTER TABLE purchase_order_items DROP COLUMN subtotal;
ALTER TABLE purchase_order_items ADD COLUMN subtotal NUMERIC(15,2)
  GENERATED ALWAYS AS (
