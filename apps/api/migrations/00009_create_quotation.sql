...
-- ============================================================
-- 00009 — fn_create_quotation (atomic create from FE wizard)
-- ============================================================
-- Single callable function untuk Step 4 submit "Buat Penawaran".
-- Atomic: generate quotation_no + INSERT header + INSERT items
-- (+ optional shipping line). Snapshots company_client_name +
-- contact_name dari master. discount_pct di items inherit via
-- existing trigger.
--
-- Items dikirim sebagai JSONB array of objects:
--   [{
...
--     "update_vendor_price":<bool|false>,
...
BEGIN
  ...
  ----------------------------------------------------------------------
  -- 2. Snapshot company_client_name + contact_name dari master
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 3. Pre-calculate totals dari items
  ----------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------
  -- 6. INSERT product items dari JSONB array
  --    discount_pct akan auto-inherit via trg_inherit_quotation_discount
  --    update_vendor_price=true akan trigger trg_sync_vendor_cost
  ----------------------------------------------------------------------
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
  ...
  ----------------------------------------------------------------------
  -- 7. INSERT shipping line (kalau ada cost) — item_type='shipping'
  --    Diskon tidak apply (CASE di GENERATED column handle exempt)
  ----------------------------------------------------------------------
  IF p_shipping_cost IS NOT NULL AND p_shipping_cost > 0 THEN
    v_line_no := v_line_no + 1;
    INSERT INTO quotation_items (
...
COMMENT ON FUNCTION fn_create_quotation(
  ...
  'Atomic create quotation: generate doc_no, snapshot client/contact name, INSERT header + items + optional shipping line. Discount + vendor cost sync handled by existing triggers. Returns new quotation_id.';
-- +goose StatementEnd
