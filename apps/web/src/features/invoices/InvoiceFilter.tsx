...
export default function InvoiceFilter({ onClose, onApply, initialValues }: InvoiceFilterProps) {
  const [createdPreset, setCreatedPreset] = useState<DatePreset>(initialValues?.createdPreset ?? DEFAULTS.createdPreset)
  const [createdStart, setCreatedStart] = useState<string>(initialValues?.createdStart ?? DEFAULTS.createdStart)
  const [createdEnd, setCreatedEnd] = useState<string>(initialValues?.createdEnd ?? DEFAULTS.createdEnd)
  const [duePreset, setDuePreset] = useState<DatePreset>(initialValues?.duePreset ?? DEFAULTS.duePreset)
  ...
  const [activeStatuses, setActiveStatuses] = useState<InvoiceStatus[]>(initialValues?.statuses ?? DEFAULTS.statuses)
  ...
  const handleApply = () => {
    onApply({
      createdPreset, createdStart, createdEnd,
      duePreset, dueStart, dueEnd,
