...
// Client header card.
export default function ClientSummaryCard({ clientName, clientInitials, clientInfo, shippingAlamat }: ClientSummaryCardProps) {
  const ci = clientInfo;
  return (
    <div>
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid rgba(204,195,216,0.2)", borderRadius: "12px", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "20px 24px", borderBottom: "1px solid rgba(204,195,216,0.15)", background: "linear-gradient(135deg, rgba(99,14,212,0.04) 0%, rgba(99,14,212,0.01) 100%)" }}>
          ...
          <div>
            <div style={{ fontSize: "16px", fontWeight: 700, color: "#111827", marginBottom: "4px" }}>{clientName}</div>
        ...
        <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(204,195,216,0.15)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", rowGap: "20px", columnGap: "24px" }}>
            ...
            <div>
              ...
              <div style={{ ...fieldValue, color: ci?.nomorTKU ? "#111827" : "#9CA3AF", fontStyle: ci?.nomorTKU ? "normal" : "italic" }}>{ci?.nomorTKU || "Belum diisi"}</div>
            </div>
            <div>
              <div style={fieldLabel}>NPWP</div>
              <div style={{ ...fieldValue, color: ci?.npwp ? "#111827" : "#9CA3AF", fontStyle: ci?.npwp ? "normal" : "italic" }}>{ci?.npwp || "Belum diisi"}</div>
            </div>
            <div>
              <div style={fieldLabel}>Reference Number</div>
              <div style={{ ...fieldValue, color: ci?.referenceNumber ? "#111827" : "#9CA3AF", fontStyle: ci?.referenceNumber ? "normal" : "italic" }}>{ci?.referenceNumber || "Belum diisi"}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
          <div>
            ...
            <div style={{ fontSize: "13px", fontWeight: 500, lineHeight: "1.6", marginTop: "4px", color: ci?.lokasi ? "#374151" : "#9CA3AF", fontStyle: ci?.lokasi ? "normal" : "italic" }}>
