...
import { PAGE_SIZE_OPTIONS, getPageNumbers } from "./helpers";
...
export default function ProductTable({ products }: ProductTableProps) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", background: "#FFFFFF", border: "1px solid rgba(204,195,216,0.2)", borderRadius: expanded ? "12px 12px 0 0" : "12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <span style={{ fontSize: "13px", fontWeight: 600, color: "#374151" }}>{products.length} produk</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                {PAGE_SIZE_OPTIONS.map(val => {
                  ...
                  return (
                    <button
                      ...
                      style={{ display: "flex", justifyContent: isActive ? "space-between" : "flex-start", alignItems: "center", padding: "4px 20px", width: "100%", height: "32px", background: "transparent", border: "none", cursor: "pointer" }}
                    >
          ...
          <div className="page-buttons">
            <button className="page-btn-nav" disabled={page === 1} onClick={() => setPage(p => Math.max(1, p - 1))}>
            ...
            <button className="page-btn-nav" disabled={page === totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))}>
              <svg width="5" height="8" viewBox="0 0 5 8" fill="none">
          ...
          >
            ...
            <svg width="10" height="6" viewBox="0 0 10 6" fill="none" style={{ transform: expanded ? "rotate(0deg)" : "rotate(180deg)", transition: "transform 0.2s ease" }}>
        ...
        <div style={{ border: "1px solid rgba(204,195,216,0.2)", borderTop: "none", borderRadius: "0 0 12px 12px", overflow: "hidden" }}>
          ...
          <div className="pagination" style={{ justifyContent: "space-between" }}>
            <span className="pagination-info">
              Menampilkan {total === 0 ? 0 : start + 1}–{Math.min(start + pageSize, total)} dari {total} Produk
            </span>
          </div>
        </div>
      )}
