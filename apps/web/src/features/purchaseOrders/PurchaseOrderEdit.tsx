...
export default function PurchaseOrderEdit({ quotationId, onNavigate, onLogout }: PurchaseOrderEditProps) {
          ...
          <div className="qe-header-section" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div className="qe-header-left">
              <nav className="qd-breadcrumb">
                <button className="qd-breadcrumb-link" onClick={() => onNavigate("purchase-orders")}>Daftar Purchase Order</button>
            ...
            <div className="qe-header-actions" style={{ display: "flex", gap: "16px", alignItems: "center" }}>
              ...
              {step === steps.length && (
                <button
                  ...
                  style={{ width: "148px", justifyContent: "center", background: "#630ED4", opacity: !isTenggatWaktuFilled || !hasContent || updateMutation.isPending ? 0.5 : 1, cursor: !isTenggatWaktuFilled || !hasContent || updateMutation.isPending ? "not-allowed" : "pointer", transition: "opacity 0.2s" }}
                  onClick={() => {
                    ...
                    const input: Omit<QuotationCreateInput, "companyClientId" | "contactId" | "status"> = {
