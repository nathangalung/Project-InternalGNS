import { type NewVendorForm, confirmModalStyle, confirmOverlayStyle } from "./helpers";
...
export default function VendorAddModal({ open, form, onChange, onClose, onSubmit, isSaving, error }: VendorAddModalProps) {
  if (!open) return null;
    ...
    <div style={confirmOverlayStyle} onClick={onClose}>
      <div style={{ ...confirmModalStyle, maxWidth: "480px" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px" }}>
          <button onClick={onClose} disabled={isSaving} style={{ padding: "8px 16px", borderRadius: "6px", border: "1px solid #D1D5DB", background: "#fff", color: "#374151", fontWeight: 600, fontSize: "14px", cursor: isSaving ? "not-allowed" : "pointer" }}>
            Batal
          </button>
          <button
            onClick={onSubmit}
            disabled={disableSubmit}
            style={{ padding: "8px 16px", borderRadius: "6px", border: "none", background: "#630ED4", color: "#fff", fontWeight: 600, fontSize: "14px", cursor: disableSubmit ? "not-allowed" : "pointer", opacity: disableSubmit ? 0.5 : 1 }}
          >
