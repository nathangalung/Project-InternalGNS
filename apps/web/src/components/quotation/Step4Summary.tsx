...
const card: React.CSSProperties = { background: "#FFFFFF", border: "1px solid rgba(204, 195, 216, 0.2)", borderRadius: "12px", padding: "24px" };
const fieldLabel: React.CSSProperties = { fontSize: "11px", color: "#6B7280", fontWeight: 600, textTransform: "uppercase", marginBottom: "4px" };
...
export default function Step4Summary({
  ...
  const hasContent = products.length > 0 || (shippingAddress.trim().length >= 20 && /[a-zA-Z]/.test(shippingAddress));
  ...
  return (
    <div className="qe-step-content">
      ...
      <div>
        <h2 className="qe-section-title" style={{ marginBottom: "12px" }}>Tenggat Waktu Penawaran</h2>
        <div style={{ ...card, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
          <div>
            <label style={{ display: "block", fontSize: "11px", fontWeight: 700, color: "#6B7280", letterSpacing: "0.5px", marginBottom: "8px", textTransform: "uppercase" }}>
              JATUH TEMPO PEMBAYARAN (HARI) <span style={{ color: "#EF4444" }}>*</span>
            </label>
            <input
              type="number" min="1" placeholder="Masukkan hari sampai jatuh tempo"
              value={jatuhTempo} onChange={(e) => setJatuhTempo(e.target.value)}
              style={{ width: "100%", background: "#F8FAFC", border: "1px solid rgba(204,195,216,0.2)", outline: "none", padding: "12px 16px", borderRadius: "8px", fontSize: "14px", color: "#111827", boxSizing: "border-box", fontFamily: "'Inter', sans-serif" }}
            />
          </div>
          <div>
            <label style={{ display: "block", fontSize: "11px", fontWeight: 700, color: "#6B7280", letterSpacing: "0.5px", marginBottom: "8px", textTransform: "uppercase" }}>
            ...
            <input
              type="number" min="1" placeholder="Masukkan jumlah hari"
              value={berlakuSampai} onChange={(e) => setBerlakuSampai(e.target.value)}
              style={{ width: "100%", background: "#F8FAFC", border: "1px solid rgba(204,195,216,0.2)", outline: "none", padding: "12px 16px", borderRadius: "8px", fontSize: "14px", color: "#111827", boxSizing: "border-box", fontFamily: "'Inter', sans-serif" }}
            />
          </div>
        </div>
        {!isTenggatWaktuFilled && (
          <div style={{ marginTop: "10px", padding: "10px 14px", borderRadius: "8px", background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.18)", fontSize: "12px", color: "#DC2626", fontWeight: 500 }}>
        ...
        <div style={{ background: "#FFFFFF", border: "1px solid rgba(204,195,216,0.2)", borderRadius: "12px", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "20px 24px", borderBottom: "1px solid rgba(204,195,216,0.15)", background: "linear-gradient(135deg, rgba(99,14,212,0.04) 0%, rgba(99,14,212,0.01) 100%)" }}>
            <div style={{ width: "48px", height: "48px", borderRadius: "12px", background: "rgba(99,14,212,0.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <span style={{ fontSize: "16px", fontWeight: 800, color: "#630ED4", letterSpacing: "-0.5px" }}>{currentClient?.initials || "—"}</span>
            </div>
            <div>
              <div style={{ fontSize: "16px", fontWeight: 700, color: "#111827", marginBottom: "4px" }}>{currentClient?.name || "-"}</div>
              <span style={{ fontSize: "12px", color: "#6B7280", fontWeight: 500 }}>{currentClient?.country || "-"}</span>
            </div>
          </div>
          ...
          <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(204,195,216,0.15)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", rowGap: "20px", columnGap: "24px" }}>
              <div>
                ...
                <div style={{ ...fieldValue, fontWeight: 600 }}>{currentClient?.narahubung || "-"}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div style={{ ...fieldValue, color: currentClient?.nomorTKU ? "#111827" : "#9CA3AF", fontStyle: currentClient?.nomorTKU ? "normal" : "italic" }}>{currentClient?.nomorTKU || "Belum diisi"}</div>
              </div>
              <div>
                <div style={fieldLabel}>NPWP</div>
                <div style={{ ...fieldValue, color: currentClient?.npwp ? "#111827" : "#9CA3AF", fontStyle: currentClient?.npwp ? "normal" : "italic" }}>{currentClient?.npwp || "Belum diisi"}</div>
              </div>
              <div>
                <div style={fieldLabel}>Reference Number</div>
                <div style={{ ...fieldValue, color: currentClient?.referenceNumber ? "#111827" : "#9CA3AF", fontStyle: currentClient?.referenceNumber ? "normal" : "italic" }}>{currentClient?.referenceNumber || "Belum diisi"}</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
            <div>
              ...
              <div style={{ fontSize: "13px", fontWeight: 500, lineHeight: "1.6", marginTop: "4px", color: currentClient?.lokasi ? "#374151" : "#9CA3AF", fontStyle: currentClient?.lokasi ? "normal" : "italic" }}>
                {currentClient?.lokasi || "Belum diisi"}
              </div>
            </div>
            <div>
              <div style={fieldLabel}>Alamat Pengiriman Barang</div>
              <div style={{ fontSize: "13px", fontWeight: 500, lineHeight: "1.6", marginTop: "4px", color: shippingAddress ? "#374151" : "#9CA3AF", fontStyle: shippingAddress ? "normal" : "italic" }}>
      ...
      {/* Ringkasan Penawaran */}
      <div>
        <h2 className="qe-section-title" style={{ marginBottom: "12px" }}>Ringkasan Penawaran</h2>

        {!hasContent && (
          <div style={{ marginBottom: "20px", padding: "10px 14px", borderRadius: "8px", background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.18)", fontSize: "12px", color: "#DC2626", fontWeight: 500 }}>
            Isi minimal satu produk atau informasi pengiriman sebelum menyimpan.
          </div>
        )}

        {/* Detail Pengiriman — hanya tampil jika pengiriman diisi */}
        {shippingAddress && <div style={{ ...card, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", marginBottom: "24px" }}>
          <div>
            <div style={fieldLabel}>WAKTU PENGIRIMAN (HARI)</div>
            <div style={{ ...fieldValue, marginTop: "4px" }}>{shippingTime ? `${shippingTime} hari` : "-"}</div>
          </div>
          <div>
            <div style={fieldLabel}>BIAYA PENGIRIMAN</div>
            <div style={{ ...fieldValue, fontWeight: 700, color: "#111827", marginTop: "4px" }}>Rp {formatRp(Number(shippingCost) || 0)}</div>
          </div>
        </div>}

        {/* Detail Produk - collapsible */}
        {products.length > 0 && <div style={{ marginBottom: "24px" }}>
          {/* Header row: label + count | pagination + toggle */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", background: "#FFFFFF", border: "1px solid rgba(204,195,216,0.2)", borderRadius: prodExpanded ? "12px 12px 0 0" : "12px", borderBottom: prodExpanded ? "1px solid rgba(204,195,216,0.15)" : "1px solid rgba(204,195,216,0.2)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ fontSize: "12px", fontWeight: 700, color: "#111827", textTransform: "uppercase", letterSpacing: "0.5px" }}>Detail Produk</span>
              <span style={{ fontSize: "12px", color: "#9CA3AF", fontWeight: 500 }}>{products.length} produk</span>
            </div>
          ...
          {/* Product cards */}
          {prodExpanded && (
            <div style={{ background: "#FFFFFF", border: "1px solid rgba(204,195,216,0.2)", borderTop: "none", borderRadius: "0 0 12px 12px", padding: "20px" }}>
